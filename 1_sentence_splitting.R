...
flora_cleaning = function(q){
  ...
  q$description = gsub("pollen cone", "pollen-cone", q$description)
  q$description = gsub("flower stem", "flower-stem", q$description)
  q$description = gsub("flowering stem", "flowering-stem", q$description)
...
##################################################
  ...
  #read in the data
  data = read.csv("Flora_of_Australia.csv", stringsAsFactors = F, encoding = "latin1", header = T)
...
write.csv(out, "Flora_of_Australia_split_sentences.csv", row.names = F, fileEncoding="Windows-1252", na = "")
